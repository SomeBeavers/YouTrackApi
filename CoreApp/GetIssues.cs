...
public class GetIssues(HttpClient client)
{
	...
	public async Task<List<YouTrackIssue>> GetIssuesAsync(string project, int limit)
	{
		var query = $"api/issues?fields=id,summary,description&query={ProjectQuery}&$top={limit}";
	...
	public async Task GetIssuesByTypeAsync()
	{
